...
#define GFX_SPI_FREQUENCY_TO_DIV(spi, freq) spiFrequencyToClockDiv((spi), (freq))
#else
#define GFX_SPI_FREQUENCY_TO_DIV(spi, freq) spiFrequencyToClockDiv((freq))
#endif
#else
#define GFX_SPI_FREQUENCY_TO_DIV(spi, freq) spiFrequencyToClockDiv((freq))
#endif
...
static void _on_apb_change(void *arg, apb_change_ev_t ev_type, uint32_t old_apb, uint32_t new_apb)
{
  ...
  {
    _spi->dev->clock.val = GFX_SPI_FREQUENCY_TO_DIV(_spi, old_apb / ((_spi->dev->clock.clkdiv_pre + 1) * (_spi->dev->clock.clkcnt_n + 1)));
    SPI_MUTEX_UNLOCK();
  }
}
...
bool Arduino_ESP32SPI::begin(int32_t speed, int8_t dataMode)
{
  ...
  if (!_div)
  {
    _div = GFX_SPI_FREQUENCY_TO_DIV(&_spi_bus_array[_spi_num], _speed);
  }
